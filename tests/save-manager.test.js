...
test('la sauvegarde v2 restaure progression, apparence et options', () => {
...
test('une principale corrompue est récupérée depuis le temporaire v2 puis réparée', () => {
  globalThis.localStorage = makeStorage();
  const manager = new SaveManager();
...
test('un temporaire corrompu laisse la sauvegarde v1 être migrée', () => {
  globalThis.localStorage = makeStorage();
  const manager = new SaveManager();
  ...
  assert.equal(JSON.parse(localStorage.getItem(manager.STORAGE_KEY)).version, 2);
  assert.equal(localStorage.getItem(manager.TEMP_KEY), null);
  assert.equal(localStorage.getItem(manager.LEGACY_KEY), null);
});
