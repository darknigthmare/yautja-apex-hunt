...
**Statut :** audit initial suivi d'un chantier corrigé ; candidat technique local validé le 22 août 2026, QA gameplay finale incomplète et publication non autorisée.
...
Le chantier a traité en priorité la jouabilité et la persistance, puis la lisibilité, l'accessibilité, la cohérence du lore et l'identité visuelle. Les preuves techniques sont consignées dans `QA_REPORT.md` : 18 tests, build, audit npm, axe et smoke test Chromium des quatre chasses ; les parcours complets victoire, défaite et persistance restent explicitement ouverts.
...
| --- | --- | --- |
| Environnement | Import principal redirigé vers `src/world/Environment.js`; le parcours hub plus quatre biomes charge les six textures dans Chromium, avec nettoyage GPU centralisé. | Inspection longue durée de la mémoire sur GPU modeste. |
...
| Art | Six WebP OpenAI 1024×1024, 1 681 184 octets, décodés et chargés en jeu. | Inspection finale des raccords 2×2 sur plusieurs écrans physiques. |
...
Aucun push GitHub ni déploiement Vercel n'est autorisé par ce document. Ils restent bloqués jusqu'à une autorisation explicite de l'utilisateur après présentation des résultats de validation.
